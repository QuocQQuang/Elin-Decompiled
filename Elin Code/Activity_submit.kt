...
import com.example.myapplication.model.SubmissionStatusResponse
...
import com.example.myapplication.model.ExpiredStatusResponse
...
class ActivitySubmit : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        checkSubmissionStatus(postId.toString(), studentId)
        checkAssignmentExpired(postId.toString())
        //checkPostDeadline(postId.toString())
        //checkPostDeadlineAndSubmissionStatus(postId.toString(), studentId)
        ...
        binding.isSub.setOnClickListener {
            val intent = Intent(this@ActivitySubmit, activity_sub::class.java)
            intent.putExtra("postId", postId)
            intent.putExtra("isSubmitted", true)
            startActivity(intent)
        }
        binding.notSub.setOnClickListener {
            val intent = Intent(this@ActivitySubmit, activity_sub::class.java)
            intent.putExtra("postId", postId)
    ...
    private fun checkSubmissionStatus(assignmentId: String, studentId: String) {
        val call = apiService.getSubmissionStatus(assignmentId, studentId)
        call.enqueue(object : Callback<SubmissionStatusResponse> {
            override fun onResponse(call: Call<SubmissionStatusResponse>, response: Response<SubmissionStatusResponse>) {
                if (response.isSuccessful) {
                    val submissionStatus = response.body()
                    if (submissionStatus != null && submissionStatus.submitted) {

                        // ẩn EditText và Button nộp bài
                        //binding.inpLink.visibility = View.GONE
                       // binding.button.visibility = View.GONE
                        binding.submissionStatusTextView.visibility = View.VISIBLE
                        // Hiển thị thông báo đã nộp bài
                        Toast.makeText(this@ActivitySubmit, "Bạn đã nộp bài!", Toast.LENGTH_SHORT).show()
                    } else {
                        // Nếu chưa nộp bài, không cần làm gì cả
                    }
                } else {
                    Log.e(TAG, "Error checking submission status: ${response.code()}")
                }
            }

            override fun onFailure(call: Call<SubmissionStatusResponse>, t: Throwable) {
                Log.e(TAG, "Failed to check submission status: ${t.message}")
            }
        })
    }
    private fun checkAssignmentExpired(assignmentId: String) {
        val call = apiService.checkAssignmentExpired(assignmentId)
        call.enqueue(object : Callback<ExpiredStatusResponse> {
            override fun onResponse(call: Call<ExpiredStatusResponse>, response: Response<ExpiredStatusResponse>) {
                if (response.isSuccessful) {
                    val expiredStatus = response.body()
                    if (expiredStatus != null && expiredStatus.expired) {


                        //  ẩn EditText và Button nộp bài, hiển thị thông báo
                        binding.inpLink.visibility = View.GONE
                         binding.button.visibility = View.GONE
                        binding.expiredMessage.visibility = View.VISIBLE
                        Toast.makeText(this@ActivitySubmit, "Bài tập đã hết hạn!", Toast.LENGTH_SHORT).show()
                    } else {
                        // Bài tập vẫn chưa hết hạn, không cần làm gì cả
                    }
                } else {
                    Log.e(TAG, "Error checking assignment expiration status: ${response.code()}")
                }
            }

            override fun onFailure(call: Call<ExpiredStatusResponse>, t: Throwable) {
                Log.e(TAG, "Failed to check assignment expiration status: ${t.message}")
            }
        })
    }

...
public class FactionBranch : EClass
{
	...
	public void DailyOutcome(VirtualDate date)
	{
		...
		{
			Chara m = member;
			if (EClass.rnd(EClass.debug.enable ? 2 : (360 * members.Count)) == 0 && !m.IsPC && EClass.pc.faction.IsGlobalPolicyActive(2712) && m.things.Find((Thing t) => t.id == "panty" && t.c_idRefCard == m.id) == null && !m.things.IsFull())
			{

...
public class Chara : Card, IPathfindWalker
{
	...
	public bool UseAbility(Act a, Card tc = null, Point pos = null, bool pt = false)
	{
		...
		void ForeachEnemy(Action<Chara> action)
		{
			if (_pts.Count == 0)
			{
				for (int num7 = EClass._map.charas.Count - 1; num7 >= 0; num7--)
				{
					Chara chara = EClass._map.charas[num7];
			...
			for (int num8 = _pts.Count - 1; num8 >= 0; num8--)
			{
				action(_pts[num8]);
			}
		}
		void ForeachParty(Action<Chara> action)
		{
			if (_pts.Count == 0)
			{
				if (IsPCParty)
				{
					for (int num9 = EClass.pc.party.members.Count - 1; num9 >= 0; num9--)
					{
						Chara chara2 = EClass.pc.party.members[num9];
						if (chara2 == this || chara2.host != null || CanSeeLos(chara2))
						{
							_pts.Add(chara2);
						}
					}
				...
				else
				{
					for (int num10 = EClass._map.charas.Count - 1; num10 >= 0; num10--)
					{
						...
						if ((chara3 == this || (chara3.IsFriendOrAbove(this) && CanSeeLos(chara3))) && (chara3 == tc || _pts.Count < 3 || EClass.rnd(_pts.Count * _pts.Count) > 6))
						{
							_pts.Add(chara3);
						}
					}
